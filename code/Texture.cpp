...
int GetTexture(string fileName)
{
    ...
    if (bitmapFile == NULL)
    {
        printf("Error: Failed to open the file at path %Second\n", fileName.c_str());
        delete[] bitmapFile;
        return -1;	// error
    }
    ...
    {
        printf("Error: Trying to load %Second it is in an invalid bitmap format.\n", fileName.c_str());
        fclose(bitmapFile);
        return -1;	// error
    }
